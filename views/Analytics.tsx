...
const Analytics: React.FC<Props> = ({ onNavigate }) => {
  ...
  const fetchAnalytics = async (userId: string) => {
    try {
      ...
      // Calculate General Accuracy (Total Correct / Total Questions)
      // This relies on 'themes' being cumulative (Initial + Reviews)

      // We will perform the calculation after fetching themesData logic below.
      // Resetting here just in case.
      // setGeneralAccuracy(null); 

      // NOTE: Removed previous logic that calculated average of review percentages.
      // We now strictly use Sum(Correct) / Sum(Total) from themes.
      ...
      if (themesData) {
        // --- General Accuracy Calculation Update ---
        // Requirement says General Accuracy, usually related to performance.
        // Keeping the logic "Total Correct / Total Questions" from Themes is valid for "Global Knowledge",
        // But "Reviews Average" (above) is valid for "Recent Performance".
        // Let's stick to the Themes Aggregation for general accuracy as it covers all history better if stored on theme.
        // Actually, let's keep the block I just replaced for `reviewsData` usage just for Effort, but...
        // Wait, the previous code overwrote `setGeneralAccuracy` with themes data later.
        // I will preserve the Themes-based accuracy calculation below, but use `reviewsData` for Effort.
